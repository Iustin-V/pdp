...
import {emailValidation, messageValidation, nameValidation, phoneValidation,} from "../../inputsValidations";
...
export const Contact = () => {
    ...
    const [valueText, setValueText] = useState({name: '', email: '', telefon: '', curs: '', mesaj: '', sendButton: ''})
    ...
    const optionCourses = allLinkCourses?.map((course) => {
        return (
            <option value={course.title} selected={course.title === localStorage.course}>
                {course.title}
            </option>
        );
    });
        ...
        <ContactFormBackground backgroundImage={background}>
            <StyledContactForm>
                <TextsSection>
                    ...
                    <ParagraphItalicStyled>{textSection.subTitle[0]}</ParagraphItalicStyled>
                </TextsSection>
                <StyledForm
